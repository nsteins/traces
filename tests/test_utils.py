...
def test_duration_to_number():
    for tdelta in timedelta_list:
        ...
        with raises(NotImplementedError):
            utils.duration_to_number(tdelta, 'hours')
...
def test_convert_args_to_list():
    ...
    with raises(TypeError):
        utils.convert_args_to_list([2, 3, 4])

    with raises(TypeError):
        utils.convert_args_to_list([2])
...
def test_datetime_floor():
    ...
    with raises(ValueError):
        utils.datetime_floor("2016-6-7")
        
    with raises(ValueError):
        utils.datetime_floor(test_dt, 'sleconds', n_units=3)


def test_weekday_number():
    assert utils.weekday_number(5) == 5
    with raises(ValueError):
        utils.weekday_number(7)
    assert utils.weekday_number("Tuesday") == 1
    with raises(ValueError):
        utils.weekday_number('Mooday')
