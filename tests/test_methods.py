...
def test_mean_interpolate():
    ...
    ts[1] = 0
    ts[3] = 20
    assert ts.mean(0, 2, interpolate='linear') == approx(2.5)
    ...
    mask[1] = True
    mask[3] = False
    assert ts.mean(0, 2, mask=mask, interpolate='linear') == approx(10/3.0)
    assert ts.mean(0, 3, mask=mask, interpolate='linear') == approx(8.0)
...
def test_sample():
    ...
    with raises(ValueError):
        ts.sample(-1, time_list[0], time_list[-1])
        
    with raises(ValueError):
        ts.sample(20, time_list[0], time_list[-1])

    # Check second and third arguments
    with raises(ValueError):
        ts.sample(1, time_list[3], time_list[0])
    ...
    with raises(ValueError):
        ts.sample(0.5, -traces.inf, 8)
    with raises(ValueError):
        ts.sample(0.5, 1, traces.inf)
...
def test_moving_average():
    ...
    with raises(ValueError):
        ts.moving_average(1, -1, time_list[0], time_list[-1])
    ...
    with raises(ValueError):
        ts.moving_average(-1, 1, time_list[0], time_list[-1])
    with raises(ValueError):
        ts.moving_average(20, 1, time_list[0], time_list[-1])

    # Check third and fourth arguments
    with raises(ValueError):
        ts.moving_average(1, 1, time_list[3], time_list[0])
